...
from swarmcore import Agent, Swarm, enable_logging
...
hooks = enable_logging()
swarm = Swarm(flow=researcher >> (analyst | writer) >> editor, hooks=hooks)
...
async def main() -> None:
    ...
    for r in result.history:
        print(
            f"  {r.agent_name}: {r.duration_seconds}s, {r.token_usage.total_tokens} tokens"
        )
